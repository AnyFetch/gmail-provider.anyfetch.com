...
/**
 * Called when imap sends a message.
 * @param googleAccount Current google account mail, used to generate URL
 * @param msg imap stream message
 * @param cb Cb with correct document
 */
var onMessage = function(googleAccount, msg, cb) {
  ...
  parser.on("end", function(mail_object) {
    var xGmThrid = "";
    var url = "";
    if('x-gm-thrid' in attrs) {
      xGmThrid = attrs['x-gm-thrid'];
      url = gmailUrl(googleAccount, xGmThrid);
    }

    var xGmMsgid = null;
    if('x-gm-msgid' in attrs) {
      xGmMsgid = attrs['x-gm-msgid'];
    }

    var xGmLabels = "";
    if('x-gm-labels' in attrs) {
      xGmLabels = attrs['x-gm-labels'];
    }
...
module.exports = function(refreshToken, googleAccount, fromUid, mailCb, finalCb) {
  ...
  async.waterfall([
    ...
    function(imap, cb) {
      ...
      imap.openBox('[Gmail]/Tous les messages', true, function(err) {
        if(err) {
          console.log(err)
        }
        cb(err, imap);
    ...
    function(imap, results, cb) {
      ...
      f.on('message', function(msg) {
        onMessage(googleAccount, msg, function(task) {
          lastUid = task.uid;
          mailCb(task);
        });
      });
