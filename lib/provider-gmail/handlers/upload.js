...
// Upload `mails` onto cluestr, then call `cb`.
var account_upload = function(mails, cb) {
  var stack = [];
  ...
  Token.find({}, function(err, tokens) {
    ...
    for(var i = 0; i < tokens.length; i += 1) {
      ...
      stack.push(function(cb) {
        ...
        retrieve(token.googleTokens.refresh_token, token.googleAccount, token.mailsSaved, function(mails) {
          // Once the mails have been retrieved,
