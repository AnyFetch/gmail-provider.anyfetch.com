 ...
 */
var getAllInbox = function(imap, cb) {
  ...
  imap.getBoxes(prefix + "/", function(err, boxes) {
    ...
    for(var key in boxes[prefix].children) {
      if(listBoxGmail[key].attribs.indexOf("\\All") !== -1) {
...
var retrieveGmail = function(refreshToken, googleAccount, fromUid, mailCb, finalCb) {
  ...
  var onMessage = function(msg, seqno) {
  ...
  async.waterfall([
    ...
    function(token, accessToken, cb) {
      ...
      imap.once('ready', function(err) {
        ...
        getAllInbox(imap, function(err, boxName) {
          if (err) {
            return finalCb(err);
          }

          imap.openBox(boxName, true, function(err) {
            if (err) {
              return finalCb(err);
            }
