...
// For each message, `mailCb` will be called with data as parameters.
...
var retrieveGmail = function(refreshToken, googleAccount, cursor, next)  {
  ...
  async.waterfall([
    ...
    function retrieveMails(token, accessToken, cb) {
      ...
      console.log('python2', gmailScript, googleAccount, accessToken, cursor.reverse ? cursor.minUid : cursor.uid, cursor.reverse ? 'reverse': '');
      var proc = spawn('python2', [gmailScript, googleAccount, accessToken, cursor.reverse ? cursor.minUid : cursor.uid, cursor.reverse ? 'reverse': '']);
      ...
      proc.stdout.on('data', function(chunk) {
        ...
        if(mails.length > 0) {
          ...
          mails.forEach(function(mail) {
            ...
            console.log("UID", cursor.uid,  "MINUID", cursor.minUid);
          });
