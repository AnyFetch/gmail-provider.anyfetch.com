...
var openInbox = function(cb) {
  ...
  console.log("Select of the mailbox");
...
var recursion = function(refreshToken, googleAccount, lastUid, mailCb, finalCb, levelOfRecursion){
  retrieveGmail(refreshToken, googleAccount, lastUid, 
  function(task){
    mailCb(task);
  },
  function(lastUidFinal){
    //If we have done with the emails
    if(lastUidFinal === -1){
      recursion--;
      finalCb(-1);                        
    }
    else {
      recursion(refreshToken, googleAccount, lastUidFinal, levelOfRecursion);
    }
  }, levelOfRecursion++);
}
...
var retrieveGmail = function(refreshToken, googleAccount, fromUid, mailCb, finalCb, levelOfRecursion) {
  ...
  var recurseFinalCb = function(finished) {
                ...
                idTimer = setTimeout(function(){
                  ...
                  if(levelOfRecursion == 0) {
                    retrieveGmail(refreshToken, googleAccount, lastUid, function(task){
                      ...
                      recurseFinalCb(finished);
                    }, levelOfRecursion);
                  }
                  else {
                    console.log(levelOfRecursion);
                    finalCb(false);
                  }
