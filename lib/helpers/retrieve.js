...
var getInboxAllMessages = function(imap, cb) {
  ...
  imap.getBoxes(prefix + "/", function(err, boxes) {
    ...
    cb(new Error("No inbox all messages detected"));
  });
};
...
var retrieveGmail = function(refreshToken, googleAccount, fromUid, mailCb, finalCb) {
  ...
  // Store the numero of the current mail treat
  var seqno = null;
  ...
  var isFinsih = function() {
    return (seqno - totalNumberOfMail + 1 === fromUid);
  };
  ...
  // Create a domain to intercept the imap parser error
  ...
  errorHandler.on('error', function(err){
    ...
    console.log("Restart retrieveGmail at uid : ", lastUid + 1);
    retryAfterTimeout();
  });
    ...
    async.waterfall([
      ...
      function newImapConnection(token, accessToken, cb) {
        ...
        imap.once('end', function(){
          console.log("end of imap if the fetch are not finished the timeout will be throw.");
          if(!isFinsih()) {
            clearTimeout(idTimer);
            retryAfterTimeout();
          }
      ...
      function retrieveAllMessages(mailbox, cb) {
        imap.search([['UID', fromUid + ':*']], function(err, results) {
          ...
          f.on('message', function(msg, seqnumber){
            seqno = seqnumber;
      ...
      function(err) {
        clearTimeout(idTimer);
        if(err) {
          console.log("Error end of the retrieveGmail : ", err);
        }
        console.log("End of the fetch of the message");
        return finalCb(err, lastUid);
      });
