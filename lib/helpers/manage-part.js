...
module.exports = function managePart(mail, part, document, attachments, serviceData, oauth2Client, cb) {
  ...
  if(part.filename !== '' && part.body.attachmentId && part.body.size !== 0) {
    ...
    googleapis.gmail('v1').users.messages.attachments.get(options, function(err, res) {
      ...
      var attachment = {
        document: {
          ...
          actions: {
            show: "https://mail.google.com/mail/b/" + serviceData.accountName + "/#search/" + encodeURIComponent(part.filename) + "/" + mail.id
          },
          metadata: {
            path: '/' + part.filename,
    ...
    if(part.mimeType === 'text/html') {
      mail.data.html = part.body.data ? (new Buffer(part.body.data, 'base64')).toString() : '';
    }
    else if(part.mimeType === 'text/plain') {
      mail.metadata.text = part.body.data ? (new Buffer(part.body.data, 'base64')).toString().replace(/</g,'&lt;').replace(/>/g,'&gt;') : '';
    }
