  ...
  if(part.filename !== '' && part.body.attachmentId && part.body.size !== 0) {
    ...
    mail.payload.headers.forEach(function walkHeaders(header) {
      switch(header.name) {
        case "From":
        case "To":
          buffers[header.name.toLowerCase()][header.value] = true;
          break;
        case "Subject":
          mailMetadata.subject= header.value;
          break;
        case "Date":
          mailMetadata.date = new Date(header.value);
          break;
      }
    });

    mailMetadata.from =  Object.keys(buffers.from);
    mailMetadata.to =  Object.keys(buffers.to);
