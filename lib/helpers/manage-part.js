  ...
  if(part.filename !== '' && part.body.attachmentId && part.body.size !== 0) {
    ...
    mail.payload.headers.forEach(function walkHeaders(header) {
      switch(header.name) {
        case "From":
        case "To":
          buffers[header.name][header.value] = true;
          break;
        case "Subject":
          mailMetadata.subject = mailData.subject = header.value;
          break;
        case "Date":
          mailMetadata.date = mailData.date = new Date(header.value);
          break;
      }
    });

    mailMetadata.from = mailData.from = Object.keys(buffers.From);
    mailMetadata.to = mailData.to = Object.keys(buffers.To);

    // Is it the content of the mail ?
    if(part.mimeType === 'text/html') {
      mailData.html = (new Buffer(part.body.data, 'base64')).toString();
    }
    else if (part.mimeType === 'text/plain') {
      mailMetadata.text = mailData.text = (new Buffer(part.body.data, 'base64')).toString().replace(/</g,'&lt;').replace(/>/g,'&gt;');
    }

    if(!document.metadata.messages) {
      document.metadata.messages = document.data.messages = [];
    }
    document.metadata.messages.push(mailMetadata);
    document.data.messages.push(mailData);
