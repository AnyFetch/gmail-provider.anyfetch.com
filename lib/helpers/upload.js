...
function mergeThreadDate(mails) {
  ...
  return "No mails in conversation";
}
...
function mergeThreadParticipants(mails) {
  var participantsBuffer = {};
  mails.forEach(function walkMails(mail) {
    mail.payload.headers.forEach(function walkHeaders(header) {
      if(header.name === 'From' || header.name === 'To') {
        var participants = decomposeMails(header.value);
        participants.forEach(function addToBuffer(participant) {
          participantsBuffer[participant.email] = participant.name || false;
        });
      }
    });
  });
  var merged = [];
  Object.keys(participantsBuffer).forEach(function assemble(email) {
    var name = participantsBuffer[email];
