...
function mergeThreadParticipants(mails) {
  var buffer = {};
  mails.forEach(function walkMails(mail) {
    mail.to.concat(mail.from).forEach(function walkAddresses(obj) {
      buffer[obj.address] = obj.name || false;
    });
  });
...
var retrieveThreadDetails = function(thread, anyfetchClient, serviceData, oauth2Client, finalCb) {
  async.waterfall([
    ...
    function finishMerge(attachments, document, cb) {
      document.metadata.messages = mergeThreadParticipants(document.metadata.messages);
      cb(null, attachments, document, cb);
    },
