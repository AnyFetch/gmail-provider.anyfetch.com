...
function mergeThreadDate(mails) {
  if(mails[0] && mails[0].headers) {
    var date = "";
    mails[0].headers.forEach(function walkHeaders(header) {
      if(header.name === 'Date') {
...
function mergeThreadParticipants(mails) {
  ...
  mails.forEach(function walkMails(mail) {
    mail.headers.forEach(function walkHeaders(header) {
      if(header.name === 'From' || header.name === 'To') {
        participantsBuffer[header.value] = true;
      }
    });
  });
  return Object.keys(participantsBuffer);
}
